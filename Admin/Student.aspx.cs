    ...
    public partial class Student : System.Web.UI.Page
    {
        ...
        private void GetStudents()
        {
            DataTable dt = fn.Fetch(@"Select ROW_NUMBER() OVER(ORDER BY (SELECT 1)) as [Sr.No], s.StudentId, s.[Name], s.DOB, s.Gender, s.Mobile,
                 s.[Address], s.RollNo, c.ClassId, c.ClassName from Student s inner join Class c on c.ClassId = s.ClassId");
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                fn.Query("Update Student set Name = '" + name.Trim() + "', Mobile = '" + mobile.Trim() + "', Address = '" +
                    address.Trim() + "', RollNo = '" + rollNo.Trim() + "', ClassId = '"+ classId +"' where StudentId = '" + studentId + "' ");
